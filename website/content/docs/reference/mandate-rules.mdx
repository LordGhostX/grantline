...
Native amounts are integer units of the native token. On the current X Layer testnet, OKB uses 18 decimals, so `1000000000000000000` represents `1 OKB`. USD limits use the same integer convention with `1e18` representing one USD. These are display conversions only; contract calls and signatures use the raw integers.
...
## Testnet behaviour and future rules

The current X Layer testnet evaluator has no USD valuation provider and is configured to skip unavailable USD quotes. USD totals can therefore be incomplete and marked with `usdLimitSkipped`; native rules and Preflight still run. A production valuation provider and external condition modules are planned, so they are not part of the fields described on this page.
