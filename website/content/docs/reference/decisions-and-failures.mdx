...
`MandateEvaluator.evaluate` returns a structured result. The decision describes what the authority layer permits next; it does not describe whether a transaction was mined or whether a downstream call succeeded.
...
The evaluator binds the digest to the current chain and evaluator address, then requires the recovered signer to match the Mandate agent. `NONE` means that no failure was found; the current evaluator returns it with `ALLOW`.
...
Native amount rules apply to the aggregate native outflow. USD rules use available valuations, and an unavailable quote sets `usdLimitSkipped` when the evaluator is configured to skip it. The current deployed testnet stack has no USD provider and skips unavailable quotes, so this path is visible in the result rather than treated as a live valuation.
