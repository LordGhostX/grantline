...
The current contracts accept an ordered `ActionPlan` containing one or more typed actions. The only supported action is `TRANSFER` version `1`; the shape below is the contract interface, not a promise of a public SDK.
...
`TRANSFER` version `1` is the current action surface. The executor performs native transfers or token transfers in array order, and a failed action reverts the complete transaction. Other action types, a public SDK, and a public API are deferred; integrations should not infer their shapes from this page.
