...
Grantline separates the agent's signed proposal from the Vault that holds capital. The evaluator decides whether the proposal is authorised, the executor translates an authorised plan into Vault calls, and the Vault accepts calls only from its configured authority. This is the contract boundary that prevents an agent's signing key from becoming direct Vault authority.
...
`MandateEvaluator` is the decision boundary. It checks the [Mandate](/docs/concepts/mandates) and active lineage, the agent binding, the signature, the deadline, the complete [Action Plan](/docs/concepts/action-plans), amount rules, and [Preflight](/docs/concepts/preflight) state. It returns `ALLOW`, `ESCALATE`, or `DENY` with the first relevant failure code; it never moves capital itself.
