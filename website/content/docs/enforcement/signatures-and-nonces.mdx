...
Grantline uses EIP-712 to sign the complete [Action Plan](/docs/concepts/action-plans) rather than an unstructured message. The digest is computed by `ActionSignature` and verified by `MandateEvaluator` before the executor can consume a nonce or call the [Vault](/docs/concepts/vaults).
