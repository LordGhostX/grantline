...
Revocation changes a Mandate from `ACTIVE` to `REVOKED` and records the revocation time. It does not delete the Mandate, its parent reference, its agent address, or the events that explain its history. It is the current contract's way to stop a Mandate; pausing and validity windows are planned roadmap items.
