...
The Next.js and Fumadocs foundation is working. The landing page is implemented in `src/app/page.tsx`; the docs shell, MDX source loader, search, LLM routes, favicon, robots route, sitemap, and Vercel Web Analytics are implemented. Phase 1 documentation under `content/docs/` includes the landing page and eight current-MVP concept pages. Phase 2 adds enforcement and execution pages for the contract boundary, signatures, nonces, escalation, revocation, transaction lifecycle, the `execution/networks/` section, and deployment wiring. Phase 3 adds read-only integrator guides and exact reference pages for Mandate rules, Action Plans, decisions, failures, and events. Phase 4 adds one consolidated roadmap page covering mandate lifecycle and policy controls, Guardians, SDK/API integration, and indexing with Decision Receipts.
