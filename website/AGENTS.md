...
Deferred website work includes connecting live demo actions to safe read-only or testnet flows, adding social preview assets, and connecting records or receipt views after the API and indexer exist. The single roadmap page intentionally describes responsibilities rather than final interfaces because no Guardian, Grantline SDK, public API, indexer, or assembled Decision Receipt exists yet. The current integrator guides use raw contract calls and external wallet tooling for the same reason. Do not add wallet signing, capital movement, or production transaction submission to the landing page without an explicit product and security review.
