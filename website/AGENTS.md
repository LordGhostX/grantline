...
Use `next/link` for internal navigation such as `/docs`; use normal anchors for in-page hash links and external destinations. Keep the canonical landing page's authorisation examples clearly illustrative until they are connected to live records or contract evidence. The `/v2` route is an isolated noindex validation preview and must remain out of the sitemap until it is deliberately promoted. Do not present placeholder `href="#"` actions as live product integrations when those CTAs are made functional. The landing page and docs shell include Vercel Web Analytics at the root layout; keep it independent of the server-only site URL configuration.
