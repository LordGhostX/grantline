...
The Next.js and Fumadocs foundation is working. The landing page is implemented in `src/app/page.tsx`; the docs shell, MDX source loader, search, LLM routes, favicon, robots route, sitemap, and Vercel Web Analytics are implemented. Phase 1 documentation under `content/docs/` includes the landing page and eight current-MVP concept pages. Phase 2 adds enforcement and execution pages for the contract boundary, signatures, nonces, escalation, revocation, transaction lifecycle, the `execution/networks/` section, and deployment wiring. Phase 3 adds read-only integrator guides and exact reference pages for Mandate rules, Action Plans, decisions, failures, and events.
...
Deferred website work includes adding roadmap material, connecting live demo actions to safe read-only or testnet flows, adding social preview assets, and connecting records or receipt views after the API and indexer exist. The current integrator guides intentionally use raw contract calls and external wallet tooling because no Grantline SDK or public API exists yet. Do not add wallet signing, capital movement, or production transaction submission to the landing page without an explicit product and security review.
