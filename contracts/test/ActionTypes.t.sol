...
contract ActionTypesTest {
    function test_roundTripsNativeTransferPlan() public pure {
        ActionTypes.TransferParameters memory transfer =
            ActionTypes.TransferParameters({asset: address(0), recipient: address(0xBEEF), amount: 1 ether});
        ActionTypes.Action[] memory actions = new ActionTypes.Action[](1);
        actions[0] = ActionTypes.Action({
            actionType: ActionTypes.ActionType.TRANSFER, version: 1, parameters: abi.encode(transfer)
        });
        ActionTypes.ActionPlan memory plan =
            ActionTypes.ActionPlan({mandateId: 7, agent: address(0xA11CE), nonce: 0, deadline: 0, actions: actions});

        ActionTypes.ActionPlan memory decodedPlan = abi.decode(abi.encode(plan), (ActionTypes.ActionPlan));
        ActionTypes.TransferParameters memory decodedTransfer =
            abi.decode(decodedPlan.actions[0].parameters, (ActionTypes.TransferParameters));
        ...
        assert(decodedPlan.actions[0].actionType == ActionTypes.ActionType.TRANSFER);
    ...
    function test_roundTripsTokenTransferParameters() public pure {
        ...
        ActionTypes.TransferParameters memory transfer =
            ActionTypes.TransferParameters({asset: token, recipient: address(0xBEEF), amount: 250 ether});

        ActionTypes.TransferParameters memory decodedTransfer =
            abi.decode(abi.encode(transfer), (ActionTypes.TransferParameters));
    ...
    function test_preservesOrderedActionsInOnePlan() public pure {
        ...
        ActionTypes.ActionPlan memory plan = ActionTypes.ActionPlan({
            mandateId: 3, agent: address(0xA11CE), nonce: 11, deadline: 1_900_000_000, actions: actions
        });
        ActionTypes.ActionPlan memory decodedPlan = abi.decode(abi.encode(plan), (ActionTypes.ActionPlan));

        ActionTypes.TransferParameters memory first =
            abi.decode(decodedPlan.actions[0].parameters, (ActionTypes.TransferParameters));
        ActionTypes.TransferParameters memory second =
            abi.decode(decodedPlan.actions[1].parameters, (ActionTypes.TransferParameters));
