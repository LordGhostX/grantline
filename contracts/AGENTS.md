...
Every deployment and authority script reads `XLAYER_TESTNET_CHAIN_ID` and aborts before broadcasting when it differs from `block.chainid`. The expected Anvil chain ID is `31337`, so local script runs must override this variable; the expected X Layer testnet chain ID is `1952`. Authority changes also require `EXPECTED_VAULT_AUTHORITY_ADDRESS` to equal the Vault's current authority, with the zero address used only for a fresh Vault.
...
| Contract                   | Address                                      |
| -------------------------- | -------------------------------------------- |
| Vault (testnet)            | `0xb0e58be3324d703b3c041859b2dCE027D31012Fd` |
| MandateRegistry (testnet)  | `0x836853Ee4786afa57d41E93874cfb3A9C7BCc72F` |
| MandateEvaluator (testnet) | `0x4bf4a8D8d3de9523373E8c29d045EecfAD76bAC4` |
| VaultExecutor (testnet)    | `0x0c214b54E32394e85864e4C6fEbbEA3681fAE14E` |

The Vault bytecode matches the locally compiled artifact, and a read-only `depositToken` call against an EOA reverts with `InvalidTokenTarget(address)`. Read-backs confirm the authority and dependency wiring, zero USD provider, enabled testnet skip mode, and empty registry. The deployer key is intentionally not recorded.
