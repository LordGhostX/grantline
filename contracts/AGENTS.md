...
The current breaking architecture gives `Grantline.setAdminController` an atomic module handover path: the old and new non-upgradeable coordinators transfer and accept module ownership, the factory's future-Vault upgrade authority moves with them, and existing Vault upgrade authorities remain a separate old-coordinator batch migration. The local suite now has 181 passing tests.
