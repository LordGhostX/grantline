...
The tracked `deployments/xlayer-testnet.json` records the fresh indexed-read X Layer testnet deployment for chain ID `1952`, not a dynamic snapshot of user Vaults. It contains the Grantline, coordinator, module proxies and implementations, runtime hashes, versions, Vault template authority, and explicit disabled native-asset and swap configuration; dynamic Vault snapshots are intentionally omitted. The deployment script reads the manifest and refuses to broadcast on a different chain. The post-deployment verifier and the expanded single-pass integration flow both pass against this stack.
...
The local suite contains 185 passing tests across the facade, evaluator boundaries, delegation narrowing, custody and cross-Vault isolation, execution atomicity and reentrancy, token return conventions, escalation reservations and transitions, targeted nonce cancellation, admin separation and proxy upgrades, synchronized protocol-admin ownership, component-role and ownership checks, initialized Vault-template validation, pause controls and lineage semantics, mandate validity windows, native-asset USD valuation and USD Preflight reserve valuation, rounding and arithmetic overflow, SWAP route validation and execution, scoped Vault/Mandate/Escalation reads, incomplete Vault-interface rejection, deployment verification, and fuzzed nonce, deadline, limit, and digest properties. `TestnetIntegration.s.sol` covers the fresh-manifest guard, controller and module isolation, native and token execution, targeted cancellation, atomic rollback, reentrancy, escalation reservation and approval, nested delegation, inherited Preflight, revocation, admin-coordinated upgrades, scoped index retention across reassignment and finalised records, cleanup, and final manifest verification; the expanded flow completed successfully on X Layer testnet after a fresh deployment. The latest source-focused coverage is 85.98% lines, 85.92% statements, 54.93% branches, and 99.57% functions across `contracts/src`. Solhint passes with only its offline update-check warning; Slither 0.11.6 reports the remaining intentional forwarded-transfer, strict-equality, bounded external-loop, benign reentrancy, adapter accounting, timestamp, optional zero-configuration, and ignored Chainlink round-metadata findings. SWAP v1, targeted nonce cancellation, native-asset USD limits, and USD Preflight reserve floors are implemented in the current source tree, while the X Layer deployment keeps the adapter and feed disabled according to its manifest.
...
- Repeat the single-pass integration flow against a fresh deployment when local end-to-end evidence is needed; the current X Layer testnet evidence is recorded in `deployments/xlayer-testnet.json`, and the script will reject a stack that already contains Vaults.
