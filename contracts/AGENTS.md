...
The workspace contains the Foundry toolchain smoke test, the `DeploymentProbe`, the `Vault`, the `MandateRegistry`, `MandateEvaluator`, `EscalationManager`, and `VaultExecutor`. The local Foundry suite covers 87 tests with Foundry 1.7.1; Anvil responds on chain ID `31337`, and the X Layer RPC responds on chain ID `1952`.
