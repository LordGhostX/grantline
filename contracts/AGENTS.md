...
The workspace contains the Foundry toolchain smoke test, the `DeploymentProbe`, the `Vault`, the `MandateRegistry`, `MandateEvaluator`, `EscalationManager`, and `VaultExecutor`. The local Foundry suite covers 115 tests with Foundry 1.7.1; Anvil responds on chain ID `31337`, and the X Layer RPC responds on chain ID `1952`.
...
`deployments/xlayer-testnet.json` is the current Phase 2 deployment record. Run `forge script script/VerifyDeployment.s.sol:VerifyDeployment --rpc-url xlayer_testnet` after loading `.env` to confirm the live addresses, runtime hashes, owner, authority, dependency wiring, zero USD provider, enabled testnet skip mode, and registry state. The local Phase 3 delegation changes alter the non-upgradeable registry ABI, so they are not reflected in this manifest or live stack; a later deployment must replace the full stack and update the manifest atomically after the SOL audit. The deployer key is intentionally not recorded.
