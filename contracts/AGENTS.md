...
The tracked `deployments/xlayer-testnet.json` is the verified X Layer testnet record for chain ID `1952`, not a record of the removed legacy stack. It contains the Grantline, coordinator, module proxies and implementations, runtime hashes, versions, Vault template authority, and three exercised Vault records. The deployment script reads the manifest and refuses to broadcast on a different chain; the fresh deployment and single-pass integration flow completed successfully, and `VerifyGrantlineDeployment` passed against the final live state.
...
`DeployGrantline` creates the Grantline proxy, internal module proxies, Vault implementation, and factory in one broadcast flow. `VerifyGrantlineDeployment` checks the manifest-defined chain, proxy implementations, runtime hashes, initialisation, UUPS identifiers, ownership, authority, module wiring, factory state, and every live Vault enumerated from the factory. Future manifests contain deployment identity and wiring only; they do not copy dynamic `vaultCount` or `vaults` state. It also exposes `runWithManifest` for in-memory unit fixtures; production verification uses `DEPLOYMENT_MANIFEST_PATH`.
...
The protocol preserves effective ancestor authority, narrower delegation, current-state re-evaluation, native amount bounds, native-balance Preflight, `ALLOW`/`ESCALATE`/`DENY` outcomes, complete-plan escalation reservations, nonce replay protection, targeted cancellation of unused and unreserved nonces, atomic typed execution, and shared reentrancy protection across normal and escalated entrypoints. The exact Mandate agent or current Vault controller can cancel through Grantline even while authority is paused, outside its validity window, or revoked; parent agents cannot cancel child nonces, and the Registry repeats the authority check before consuming the nonce. USD valuation is deliberately absent from the first-release ABI until a reliable production design exists. Protocol-admin operations deliberately live on `GrantlineAdmin`: `setVaultImplementation` changes only future Vault proxies, `upgradeVault` changes one existing Vault, `upgradeModules` changes internal module proxies, and `setVaultController` changes one Vault's controller assignment. Grantline remains the Vault owner for custody and pause controls, while Vaults store the admin coordinator separately as their UUPS upgrade authority.
...
The local suite contains 140 passing tests across the facade, evaluator boundaries, delegation narrowing, custody and cross-Vault isolation, execution atomicity and reentrancy, token return conventions, escalation reservations and transitions, targeted nonce cancellation, admin separation and proxy upgrades, synchronized protocol-admin ownership, component-role and ownership checks, initialized Vault-template validation, pause controls and lineage semantics, mandate validity windows, SWAP route validation and execution, incomplete Vault-interface rejection, deployment verification, and fuzzed nonce, deadline, limit, and digest properties. `TestnetIntegration.s.sol` covers the fresh-manifest guard, controller and module isolation, native and token execution, targeted cancellation, atomic rollback, reentrancy, escalation reservation and approval, nested delegation, inherited Preflight, revocation, admin-coordinated upgrades, cleanup, and final manifest verification; the fresh X Layer run completed successfully and its final manifest passed `VerifyGrantlineDeployment` before SWAP v1 and nonce cancellation were added. The last source-focused `forge coverage --ir-minimum --exclude-tests` report reached 86.43% line coverage, 59.51% branch coverage, and 98.96% function coverage across `contracts/src`; deployment and integration scripts remain outside this measurement. Solhint passes with only its offline update-check warning; Slither reports the remaining intentional forwarded-transfer, strict-equality, bounded external-loop, benign reentrancy, adapter accounting, and timestamp findings. SWAP v1 and targeted nonce cancellation are local-only until a fresh manifest-configured deployment and a later integration pass are approved; the current X Layer stack remains unchanged and does not expose either capability.
