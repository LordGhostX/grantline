...
abstract contract ScriptBase {
    ...
    error ManifestAddressMismatch(
        string component,
        address expectedAddress,
        address actualAddress
    );
    error ManifestCodeHashMismatch(
        string component,
    ...
    function _requireRuntimeCodeHash(
        address target,
        bytes32 expectedCodeHash,
        ...
        if (
            expectedCodeHash == bytes32(0) || actualCodeHash != expectedCodeHash
        ) {
            revert ManifestCodeHashMismatch(
                component,
                expectedCodeHash,
                actualCodeHash
            );
